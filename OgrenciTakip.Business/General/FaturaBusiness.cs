...
{
	public class FaturaBusiness : BaseHareketBusiness<Fatura, OgrenciTakipContext>, IBaseHareketSelectBusiness<Fatura>
    {
        ...
        public IEnumerable<DateTime> FaturaDonemList(Expression<Func<Fatura, bool>> filter)
        {
            ...
            {
                x.PlanTarih
            }).GroupBy(x => x.PlanTarih).Select(x => x.Key).ToList(); //plan tarihi gruplayarak geriye dönem list döndürmüş olduk
        }
        ...
        public int MaxFaturaNo(Expression<Func<Fatura, bool>> filter)
        {
            ...
            {
                x.FaturaNo
            }).DefaultIfEmpty().Max(x => x.FaturaNo ?? 0); //DefaultIfEmpty kullanmazsak null olarak geliyorsa max'ı bulunmaz o yüzden hata verir
        }
    }
}