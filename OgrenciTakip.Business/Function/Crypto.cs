...
namespace OgrenciTakip.Business.Functions
{
	public static class Crypto
	{
		public static string Encrypt(this string sifrelenecekVeri, string anahtar)
		{
			...
			var sifrenelecekVeriToByte = Encoding.UTF8.GetBytes(sifrelenecekVeri);
			...
			var salyBytes = new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 };

			using (var ms = new MemoryStream())
			{
				using (var aes = new RijndaelManaged())
				{
					var key = new Rfc2898DeriveBytes(anahtarToByte, salyBytes, 1000);
					...
					{
						cs.Write(sifrenelecekVeriToByte, 0, sifrenelecekVeriToByte.Length);
						cs.Close();
					}
